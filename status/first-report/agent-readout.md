...
- **vocab-from-cache** — Operation/filter vocab harvested from dyld cache; `book/graph/mappings/vocab/ops.json` (196) and `filters.json` (93) status `ok`, with guardrail `check_vocab.py`. Trimmed cache copies parked under `book/graph/mappings/dyld-libs/`.
...
- **runtime-checks** — Harness now prefers `book/api/SBPL-wrapper/wrapper` (SBPL/blob). Bucket profiles run; `airlock` apply returns EPERM on this host; `bsd` works via SBPL/compiled blob. Matrix/expected shapes in `out/expected_matrix.json`; results in `out/runtime_results.json`. Needs rerun on a permissive host or adjusted profiles for reliable semantic evidence.
- **sbpl-graph-runtime** — SBPL→graph→runtime triples incomplete. `allow_all` works; strict `(deny default)` shapes kill the probe (-6) despite added allowances. Needs relaxed defaults or different harness to capture allow/deny paths; manifest not yet built.
...
- **Validation harness** (`validation/README.md`, `tasks.py`):
  ...
  - Vocabulary: active. `validation/out/vocab/{ops.json,filters.json,operation_names.json,filter_names.json}` mirrored from vocab-from-cache; status `ok`.
  - Semantic graph: blocked on runtime apply; legacy sandbox-exec logs exist (`metafilter.jsonl`, `network.jsonl`, `mach_services.jsonl`, `sbpl_params.jsonl`) but flagged brittle; `runtime_results.json` notes sandbox_init EPERM.
...
- Vocab tables (`vocab/ops.json`, `vocab/filters.json`) status `ok`.
...
- Semantic validation is the main hole: runtime apply fails for strict profiles/system blobs on this host. Options: run wrapper on a permissive host, relax profiles for probes, or extend wrapper to handle apply quirks.
