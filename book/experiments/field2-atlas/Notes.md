...
- `atlas_runtime.py` now requires the runtime-adversarial promotion packet (`out/promotion_packet.json`) for runtime_events/baseline/run_manifest paths; legacy `out/` paths only load with `--allow-legacy`.
