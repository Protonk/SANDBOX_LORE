...
- **Observations**: Payload `2560` appears only in the triple specs (both TCP/UDP, any ordering, `require-all`/`require-any`, nested) as a tag `0` node with `u16_role: filter_vocab_id` and literal `com.apple.flow-divert`. Singles and pairs stay on low vocab IDs; negative controls remain low and never produce `2560`. `anchor_filter_map.json` already lists `flow-divert` (candidates `local`/`xattr`, field2 `[2, 7, 2560]`, status `blocked`), and field2-atlas static records for field2=7 carry the same seed anchor/status. The matrix evidence is consistent and strengthens the `local` interpretation, but no mapping changes made yet.
...
- **Done**: Cross-checked against `anchor_filter_map`, field2 atlas, and tag-layout contracts; retired 2560 from the unknown-high set as characterized triple-only (tag0/u16_role=filter_vocab_id, literal `com.apple.flow-divert`) and added guardrail coverage in `book/tests/test_field2_unknowns.py`.
