...
- Generated per-op summary from adversarial runtime results via `python book/experiments/op-coverage-and-runtime-signatures/summarize_from_adversarial.py` → `out/op_runtime_summary.json`. Current counts: 12 file-read* probes (10 match, 2 VFS-related mismatches), 12 file-write* probes (10 match, 2 VFS-related mismatches mirroring reads), 8 mach-lookup probes (all match), and 4 network-outbound probes (two per profile, all match after switching the sandboxed client to `/usr/bin/nc` with startup shims).
