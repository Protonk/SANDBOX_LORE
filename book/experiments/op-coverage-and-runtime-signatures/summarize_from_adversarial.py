...
from book.api.runtime.pipeline import build_op_summary_from_index
...
def build_reference_op_summary(observations: list[runtime_events.RuntimeObservation]) -> dict:
    """
...
if canonical_op is None:
    ...
    obs_objs = [runtime_events.RuntimeObservation(**o) for o in observations]
    canonical_op = rt_map.build_op_summaries(obs_objs, world_id=WORLD_ID)
else:
    obs_objs = [runtime_events.RuntimeObservation(**o) for o in observations]
...
if isinstance(canonical_op, dict) and canonical_op.get("ops"):
    ...
    for op_name, ref_entry in reference.items():
        ...
        for field in ["probes", "matches", "mismatches"]:
...
try:
    ...
    events_index, _ = rt_map.write_per_scenario_traces(
        [runtime_events.RuntimeObservation(**o) for o in observations], traces_dir, world_id=WORLD_ID
    )
    events_index_path = mapping_root / "events_index.json"
    rt_map.write_events_index(events_index, events_index_path)

    scenario_doc = rt_map.build_scenario_summaries(
        [runtime_events.RuntimeObservation(**o) for o in observations],
        expected_doc,
        world_id=WORLD_ID,
    )
    ...
    op_doc = rt_map.build_op_summaries([runtime_events.RuntimeObservation(**o) for o in observations], world_id=WORLD_ID)
    ...
    manifest = rt_map.build_manifest(WORLD_ID, events_index_path, scenario_path, op_path)
