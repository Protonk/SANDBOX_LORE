...
- Golden triples (custom, allow-default, file-centric): `runtime:allow_all`, `runtime:metafilter_any`, `bucket4:v1_read`, plus minimal strict profile `runtime:strict_1`. SBPL mode via `runtime_harness` aligns runtime with expectations (OS perms on `/etc/hosts` writes are outside sandbox scope). Wrapper “no version specified” errors on blobs are avoided by using SBPL inputs.
...
- Status remains `partial` by design; do not treat it as flaky. The same `/tmp` → `/private/tmp` behavior is documented in `book/experiments/vfs-canonicalization/Report.md`.
