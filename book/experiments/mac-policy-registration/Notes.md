...
- Added per-policy ops evidence extraction (`ops_exec_slots`) and a manifest builder (`build_mac_policy_boot_manifest.py`), then reran the KC pass and generated `book/experiments/mac-policy-registration/out/mac_policy_boot_manifest.json`.
  - Ops evidence slot counts now: AppleImage4 (1), AMFI (1151), Quarantine (6), ASP (9 via store-chain), EndpointSecurity (8), Sandbox (192), mcxalr (80). Owner top entries in the manifest align to the expected fileset entries for each policy’s hooks.
