...
- Disassembly of the helpers is still blocked by Mach-O fileset wrapping. Direct `objdump`/`llvm-objdump` on `BootKernelExtensions.kc` ignored the fileset entry, and byte-slicing by fileoff produced “truncated/malformed object.” Next step is to extract the sandbox fileset (e.g., `kmutil emit-macho` or a custom unwrapping) and then search the helper functions for `tbz`/`tbnz`/`ubfx`/`and` masks on the payload register.
