...
- **6) Synthesis and guardrails**
  ...
  - Added a headless helper-finder script (`book/api/ghidra/scripts/find_field2_evaluator.py`) and ran it on the extracted sandbox kext (`/tmp/sandbox_arm64e/com.apple.security.sandbox`) via project `sandbox_field2_sbx`. Output lives under `dumps/ghidra/out/14.4.1-23E224/find-field2-evaluator/`: the heuristic surfaced `__read24` at `fffffe000b410ee4` (halfword+byte load with bounds checks) as the small ldrb+ldrh helper, with callers including `_eval` at `fffffe000b40d698`. Dumps (`helper.txt`/`evaluator.txt`) captured, but the helper is not yet confirmed as the field2 reader.
