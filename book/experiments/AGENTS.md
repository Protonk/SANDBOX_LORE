...
These instructions apply to all subdirectories of `book/experiments/`. Experiments share the same fixed host baseline, recorded once in `book/world/sonoma-14.4.1-23E224-arm64/world-baseline.json` (the single source of truth for host metadata).
...
* Use the validation driver  
  ...
  CARTON is the frozen IR/mapping layer (`book/graph/carton/CARTON.json`); add new experiments/outputs without mutating CARTON-listed files.
