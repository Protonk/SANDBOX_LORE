...
- Compile/decode via `book.api.sbpl_compile` and `book.api.decoder`.
- Runtime harness via `book.api.golden_runner.run_expected_matrix` (reusing `runtime-checks` shims).
