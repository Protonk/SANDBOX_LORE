...
- Added `F_GETPATH` emission to `sandbox_reader`/`sandbox_writer` so successful opens record the kernel-reported FD path on stderr; vfs-canonicalization now uses this to populate `observed_path`.
