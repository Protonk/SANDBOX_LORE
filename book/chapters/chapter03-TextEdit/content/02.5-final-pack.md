...
Each subsection below is anchored by one or more code examples in `profiles/textedit/`. The prose here is intentionally thin; later passes will expand it once the code is in place.
...
```python
# profiles/textedit/tools/02.1_capability_survey.py (excerpt)
...
Synthetic but representative fragment of `profiles/textedit/output/02.1_capability_summary.json`:
...
```python
# profiles/textedit/tools/02.1_capability_checklist.py (excerpt)
...
def print_checklist(summary: dict) -> None:
    ...
    print(
        "- user-selected files: read_write=",
        user_files.get("read_write"),
        "executable=",
        user_files.get("executable"),
    )

    ubiquity = ent.get("ubiquity", {})
    print(
        "- ubiquity/iCloud enabled:",
        ubiquity.get("enabled"),
        "containers:",
        ubiquity.get("containers"),
    )
...
```python
# profiles/textedit/tools/02.2_profiles_and_containers.py (excerpt)

def parse_container_notes(path: Path) -> Dict[str, object]:
    container_root = None
    known_subdirs: List[str] = []
    if not path.exists():
...
def summarize_profile_structure(sb_text: str) -> Dict[str, bool]:
    return {
        ...
        "has_bundle_param_rules": (
            "application_bundle" in sb_text
            or "application_bundle_id" in sb_text
        ),
        "has_application_container_rules": (
            "application_container" in sb_text
            or "application_container_id" in sb_text
        ),
    }
```

Synthetic but representative fragment of `profiles/textedit/output/02.2_profiles_and_containers.json`:
...
def format_human_shape(joined: dict) -> str:
    ...
    profile_flags = joined.get("profile_flags", {})
    flags = [
    ...
    caps = [
        ...
        f"ubiquity={ent.get('ubiquity', {}).get('enabled')}",
    ]

    return (
        f"{joined.get('bundle_id')} -> "
        f"container={container} | flags[{', '.join(flags)}] | caps[{', '.join(caps)}]"
    )
```
...
```sh
# profiles/textedit/tools/02.3_trace_operations.sh (excerpt)

# Actions -> interesting traces:
# - Open file in ~/Documents:
#     * Expect file opens in user paths under ~/Documents
#     * Expect mach-lookup to tccd (privacy) for document/location access
# - Auto-save to container:
#     * Writes under container Data/Library/Autosave Information
# - Print document:
#     * Printing service interactions; CUPS / print pipeline sockets

# (Synthetic: the script is scaffolded and should be adapted before running on a live system.)
```
...
```python
# profiles/textedit/tools/02.3_trace_annotate.py (excerpt)
...
14:32:10.123  TextEdit  open    /Users/demo/Documents/example.txt
    # user-selected file entitlement + sandbox extension
14:32:10.125  TextEdit  getattr /Users/demo/Documents/example.txt
    # user-selected file entitlement + sandbox extension
14:32:10.200  TextEdit  read    /Users/demo/Documents/example.txt
    # user-selected file entitlement + sandbox extension
14:32:10.600  TextEdit  open    /Users/demo/Library/Containers/com.apple.TextEdit/Data/Library/Autosave Information/Unsaved Document 1.rtf
    # inside app container (appsandbox-container rules)
14:32:10.900  TextEdit  write   /Users/demo/Library/Containers/com.apple.TextEdit/Data/Library/Autosave Information/Unsaved Document 1.rtf
    # inside app container (appsandbox-container rules)
```
...
```python
# profiles/textedit/tools/02.4_pattern_extraction.py (excerpt)
...
def extract_common_patterns(sb_text: str) -> Dict[str, object]:
    ...
    return {
        "filesystem_patterns": {
            "has_container_macros": (
                "appsandbox-container" in sb_text or "application_container" in sb_text
            ),
            "has_system_read_only": (
                "/System" in sb_text or "/usr/bin" in sb_text
            ),
        ...
        "network_patterns": {
            "has_network_star": (
                "network*" in sb_text or "network-outbound" in sb_text
            ),
            "has_specific_denies": (
                "usbmuxd" in sb_text or "localhost:631" in sb_text
            ),
        },
    }
```

Synthetic but representative fragment of `profiles/textedit/output/02.4_pattern_extraction.json`:
...
```json
{
  ...
  "network_patterns": {
    "has_network_star": true,
    "has_specific_denies": true
  }
}
```
...
Synthetic but representative output, highlighting three buckets:
...
appstore_rules:
  ...
  (subpath "/Library/Application Support/AppStore/Content/")              # bundle-specific support paths
