...
Contents:
- `op_table_vocab_alignment.json` – Op-table entries annotated with operation/filter IDs (via vocab) for synthetic profiles. This is the bridge between op-table buckets and SBPL operation names. Includes `metadata` for host + vocab hashes.
- `op_table_operation_summary.json` – Decoder-backed summaries from `op-table-operation` (per-profile op-count, buckets, and operation sets) with a shared `metadata` block.
- `op_table_signatures.json` – Structural signatures per op-table entry (tag counts, field2 distributions, reachable nodes). These are fingerprints for how a bucket “looks” in the graph; bundled with shared `metadata`.
