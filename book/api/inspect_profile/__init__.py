"""
Profile inspection helpers for compiled sandbox blobs.

Scope:
- Read-only summaries of compiled profiles (modern graph-based format) on the Sonoma baseline.
- Optional SBPL compile via book.api.sbpl_compile when needed by the CLI.

Key functions:
- summarize_blob(bytes) -> dict with format/op_count/section lengths/stride stats/tag counts/literal strings/decoder echo.
- load_blob(path) -> bytes helper.
"""
