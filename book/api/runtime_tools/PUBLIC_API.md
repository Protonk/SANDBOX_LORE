# `runtime_tools` public API (Sonoma 14.4.1 baseline)

This document defines the **supported, stable** public interface for `book.api.runtime_tools`.
...
- `import book.api.runtime_tools as rt`

Callers should treat **only** the symbols exported in `rt.__all__` as stable. Submodules under `book/api/runtime_tools/` exist, but are **internal/legacy** unless explicitly exported.
...
- `python -m book.api.runtime_tools ...`
