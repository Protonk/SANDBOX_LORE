# runtime_tools service contract (SPEC)

This document is the reference contract for `book/api/runtime_tools/` as a repo-level service. It is written for agents and future tooling that need reliable runtime evidence production without importing experiment code or relying on shell state.

Scope note: this is a *service contract* for evidence production and artifact IO. It does not claim global sandbox semantics; it describes what `runtime_tools` writes and what its APIs guarantee on this host baseline.
...
- lock file: `out/.runtime_tools.lock`
...
- Mapping generators remain outside runtime_tools; `runtime_tools` standardizes *inputs* via promotion packets.
