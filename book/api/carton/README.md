...
- **Host binding** – CARTON is tied to the Sonoma world baseline recorded in `world_id sonoma-14.4.1-23E224-arm64-dyld-2c0602c5`. All CARTON artifacts are generated for that baseline and checked against it.
...
- **Operation**
  ...
  - Coverage: `book/graph/mappings/carton/operation_coverage.json` (where each operation shows up in system profiles).
...
- **Profile / Profile Layer**
  ...
  - Index: `book/graph/mappings/carton/profile_layer_index.json` (operations grouped by profile identity and layer).
- **Concept bindings**
  - `book/graph/mappings/carton/concept_index.json` ties concept‑inventory names (for example, `operation`, `filter`, `profile-layer`) to the specific CARTON mappings and top‑level keys that represent them.
