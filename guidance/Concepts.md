...
- **Parameterization**  
  Parameterization uses `(param …)` forms in SBPL to template profiles so they require external input at compile time; the compiler substitutes concrete strings provided by the caller. System profiles use this to splice paths and names (often via `string-append`) without hardcoding them, but the parameters must be supplied when compiling or the profile cannot be resolved into a usable binary blob.

- **Profile Layer**  
  A profile layer describes which sandbox policy is being applied in the multi-layer system: the global **platform** policy that applies to almost all processes, per-process policies like App Sandbox or custom profiles attached via `sandbox_init*`, and any other Seatbelt profiles. The conceptual model is that multiple layers can apply to a single operation, with platform policy evaluated first and per-process policy next; thinking in terms of layers helps keep straight where a particular rule lives and why a decision was made. (See also **Policy Stack Evaluation Order** for how multiple layers are applied at runtime.)
